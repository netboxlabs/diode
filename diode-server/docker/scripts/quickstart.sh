...
info "  diode_target_override:         $DIODE_TARGET"
info "That is the $NETBOX_TO_DIODE_CLIENT_ID client secret, not the ingest one above."
info "If NetBox runs in a container, diode_target_override must be an address"
info "reachable from inside it: the host's IP or FQDN, not localhost."
