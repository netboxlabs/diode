...
def test_get_settings_page(netbox_web_client):
    ...
    assert "Settings" in response.text, "Settings page should have 'Settings' title"
    ...
    assert "Main (default)" in response.text, "Settings page should display 'Main (default)' as the branch"
...
def test_get_credentials_list_page(netbox_web_client):
    ...
    # Verify page contains expected content
    assert "credential" in response.text.lower(), "Credentials list page should mention credentials"
...
def test_add_credential_flow(netbox_web_client):
    ...
    assert response.status_code == 302, f"Expected redirect after creating credential, got {response.status_code}"

    # Check redirect location
    assert "credentials/secret" in response.headers.get("Location", ""), "Should redirect to secret page after creation"
    ...
    assert secret_response.status_code == 200, f"Expected 200 OK on secret page, got {secret_response.status_code}"
    ...
    assert client_name in secret_response.text, f"Secret page should display client name '{client_name}'"
...
def test_add_credential_requires_client_name(netbox_web_client):
    ...
    assert response.status_code == 200, f"Expected form validation error, got {response.status_code}"

    # Check for error message in response
    assert (
        "enter a name for the client credential that will be created for authentication to the diode ingestion service"
    ) in response.text.lower() or "error" in response.text.lower(), (
...
def test_delete_credential_flow(netbox_web_client, diode_client_credential):
    ...
    assert "credentials" in location, f"Should redirect to credentials list page, got: {location}"
...
def test_unauthenticated_access_redirects_to_login(test_config):
    ...
    response = session.get(f"{base_url}/plugins/diode/settings/", allow_redirects=False)

    # Should redirect to login page
    assert response.status_code == 302, f"Expected redirect for unauthenticated user, got {response.status_code}"

    assert "login" in response.headers.get("Location", "").lower(), "Should redirect to login page"
