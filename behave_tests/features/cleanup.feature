...
Scenario: Cleanup
    ...
    Given the device "undefined" is deleted
    ...
    Given the site "NA-NY" is deleted
    Given the device type "ISR4321-1" is deleted
    ...
    Given the cluster type "AWS" is deleted
    Given the cluster group "North America" is deleted
    Given the cluster group "NA" is deleted
